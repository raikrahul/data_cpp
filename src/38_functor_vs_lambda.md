...
049.:01. DRAW MEMORY LAYOUT [Stack]. Lambda `auto l = [x=10, y=20](int a){ return x+y+a; }`. Compiler generates: `class Lambda { int x; int y; operator()(int a)... }`. Layout at [0x5000]: [0x5000]=x(10) [4 bytes]. [0x5004]=y(20) [4 bytes]. Size=8 bytes. No vptr. No pointer overhead. **Why**: Lambdas are just unnamed structs with `operator()`. **Functor Comparison**: `struct Functor { int x=10, y=20; int operator()... }`. Layout: Identical. Size=8 bytes. **std::function Wrapper**: `std::function<int(int)> f = l`. Layout: [0x5000]=VTablePtr [8 bytes]. [0x5008]=SmallObjectBuffer/HeapPtr [24 bytes]. Total=32 bytes. **Why**: Type Erasure. The `std::function` must support *any* callable, so it uses virtual dispatch (indirect call). **Overhead**: 32 bytes (vs 8) + Virtual Call (vs Direct Call). **Result**: `std::function` is 400% larger and slower. Use `auto` for lambdas.
# Functor vs Lambda State Analysis
